...
merge_data <- function(directory) {
  ## read the dataset in test and train sets of data given in the assignment
  ...
  train_data <- read.table(path)
   ...
   ## read the test and training subject labels from the working directory
  ...
  ## read the activity labels from the working directory
...
## The following does a descriptive activity names to name the activities in the data set
...
melt_data_and_write_tidy_set <- function(data_set, path_to_tidyset_file) {
  ...
  ## Save the data back to the tidy_data format
...
##  check if you can read the tidyset if you wished
read_tidy_set <- function(path_to_tidyset_file) {
  tidy_set <- read.table(path_to_tidyset_file)
  
  return (tidy_set)
}
## Run the below command to read the tidyset inside the text file
