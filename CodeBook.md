...
On the below line of code of run_analysis.R,
...
These are the lines of code that will start the process of cleaning the data.

The variables at the global environment are therefore:
...
Inside the function `merge_data`, we take data from the following text files:
 ...
 - 
 - `train/X_train.txt` is the  local variable, `train_data` containing all the 541 columns of raw data in the training set
 - `activities.txt`  is  the local variable, `activity_labels` containing all the different types of activities: `WALKING`, `WALKING_UPSTAIRS`, `SITTING`, etc
 - `train/subject_train.txt`is the local variable, `subject_train` containing all the possible subject data inside the training set.
 - `test/subject_test.txt`is us the local variable, `subject_test` containing all the possible subject data inside the test set.
 - `train/y_train.txt` is the local variable, `y_train` containing all the possible label data inside the training set.
 - `test/y_test.txt` is the local variable, `y_test` containing all the possible label data inside the test set. 
...
Inside the function `extract_mean_std`, we take data from the following text files:
 ...
 - `features.txt` will the local variable, `features_data` containing all the features and their names
...
### Transformations inside melt_data_and_write_tidy_set
 ...
 U may run the script using the methods merged_data and follow the present working directory structure.
